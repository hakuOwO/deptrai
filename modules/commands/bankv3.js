...
module.exports.run = async function ({ api, args, event, Users, permssion, Currencies }) {
  ...
  const araxy = await axios.get("https://api-bank.hanguyen48.repl.co")
  if (`${araxy.data.data}` == "false") {
    return api.sendMessage("[ ADMIN ] - ĐÃ ĐẬP SV", threadID, messageID)
  } else {
    if (args[0] == '-r' || args[0] == 'register') {
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/dangky?uid=${senderID}&name=${encodeURI(lmao)}`)
    ...
    if (args[0] == 'dangkyvay' || args[0] == '-rvay') {
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/dangkyvay?uid=${senderID}&name=${encodeURI(lmao)}`)
      if (`${res.data.data}` == "gban" || `${res.data.data}` == "true") {
        return api.sendMessage(`${res.data.msg}`, threadID, messageID)
      }
    ...
    if (args[0] == 'reset' || args[0] == 'resetpass') {
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/resetpass?sender=${senderID}`)
      if (`${res.data.data}` == "gban" || `${res.data.data}` == "true") {
    ...
    if (args[0] == "all" || args[0] == "-a") {
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/all`)
    ...
    if (args[0] == "gban" || args[0] == "gbanadd") {
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/gban?uid=${args[1]}&reson=${encodeURI(args.slice(2).join(" "))}&author=${event.senderID}`)
    ...
    if (args[0] == "top") {
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/top`)
      return api.sendMessage(`${res.data.data}`, threadID, messageID)
    }
    ...
    if (!args[0]) {
      const res = await axios.get("https://api-bank.hanguyen48.repl.co/home")
...
module.exports.handleReply = async function ({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    case "find": {
      ...
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/find?luachon=${find_type}&find=${event.body}`)
    ...
    case "check": {
      ...
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/check?senderID=${senderID}&pass=${encodeURI(event.body)}`)
      if (`${res.data.data}` == "false") {
        return api.sendMessage(`${res.data.msg}`, threadID, messageID)
      } else {
    ...
    case "checkvay": {
      var uh = handleReply.thread
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/checkvay?senderID=${event.senderID}&pass=${encodeURI(event.body)}`)
      if (`${res.data.data}` == "false") {
        return api.sendMessage(`${res.data.msg}`, threadID, messageID)
      } else {
        return api.sendMessage(`${res.data.data}`, uh, () => api.sendMessage("[ BANK ] - NHẬP MẬT KHẨU ĐÃ ĐÚNG", threadID)
    ...
    case "vay": {
      ...
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/vay?senderID=${senderID}&pass=${event.body}&sotien=${money}`)
      if (`${res.data.data}` == "false") {
    ...
    case "trả": {
      var uh = handleReply.thread;
      var money = handleReply.send;
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/tra?senderID=${event.senderID}&pass=${event.body}&sotien=${money}`)
      if (`${res.data.data}` == "false") {
        return api.sendMessage(`${res.data.msg}`, uh, () => api.sendMessage(`${res.data.msg}`, threadID)
        )
      } else {
    ...
    case "send": {
      var uh = handleReply.thread;
      var money = handleReply.send;
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/send?senderID=${senderID}&sotien=${money}&pass=${encodeURI(event.body)}`)
      if (`${res.data.data}` == "false") {
        return api.sendMessage(`${res.data.msg}`, threadID, messageID)
      } else {
        await Currencies.decreaseMoney(senderID, parseInt(money));
    ...
    case "rút": {
      ...
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/rut?senderID=${senderID}&sotien=${money}&pass=${encodeURI(event.body)}`)
      if (`${res.data.data}` == "false") {
        return api.sendMessage(`${res.data.msg}`, uh, () => api.sendMessage(`${res.data.msg}`, threadID)
        )
      } else {
    ...
    case "pay_next": {
      ...
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/pay?sotien=${paymm}&sender=${event.senderID}&nguoinhan=${text3}&pass=${text2}`)
      if (`${res.data.data}` == "false") {
    ...
    case "next": {
      ...
      const res = await axios.get(`https://api-bank.hanguyen48.repl.co/changepass?sender=${event.senderID}&pass=${text1}&newpass=${text2}`)
      if (`${res.data.data}` == "false") {
        return api.sendMessage(`${res.data.msg}`, thread, () => api.sendMessage(`${res.data.msg}`, threadID)
        )
      } else {
